...
LOG_MODULE_REGISTER(MODULE, CONFIG_IFMCU_CDC_LOG_LEVEL);
...
#define USB_CDC_RX_BLOCK_SIZE CONFIG_IFMCU_BUF_SIZE
...
#if CONFIG_IFMCU_CMSIS_DAP_BULK_ENABLE
...
#endif
		if ((!IS_ENABLED(CONFIG_IFMCU_CMSIS_DAP_BULK_ENABLE) || bulk_module_ready)) {
			/* Enable USBD */
			int err;
