...
#define FULL_FW_INFO_STRING	\
	...
	"Zephyr OS v" KERNEL_VERSION_STRING "-g" STRINGIFY(BUILD_VERSION) "\r\n" \
...
#define ID_DAP_VENDOR_NRF54L_RESET	    ID_DAP_VENDOR19
...
#define NRF54L_RESET_DURATION 			K_MSEC(100)
...
static void nrf54l_reset_work_handler(struct k_work *work);
K_WORK_DELAYABLE_DEFINE(nrf54l_reset_work, nrf54l_reset_work_handler);
...
size_t dap_execute_vendor_cmd(uint8_t *in, uint8_t *out)
{
	...
	if (in[0] == ID_DAP_VENDOR_NRF54L_RESET) {
		if (!k_work_busy_get(&nrf54l_reset_work.work)) {
			k_work_reschedule(&nrf54l_reset_work, K_NO_WAIT);
		}
		goto success;
	}
...
{
	if (argc == 1 || (strcmp("nrf54l15", argv[1]) == 0)) {
		/* Reset nRF54L15 default */
		if (!k_work_busy_get(&nrf54l_reset_work.work)) {
			k_work_reschedule(&nrf54l_reset_work, K_NO_WAIT);
		}
		return 0;
...
SHELL_CMD_ARG_REGISTER(reset, NULL,
		   ...
		   "Usage: reset [target]\n"
		   "[target] - nrf54l15 or nrf52820 (default: nrf54l15)",
		   cmd_reset, 1, 1);
