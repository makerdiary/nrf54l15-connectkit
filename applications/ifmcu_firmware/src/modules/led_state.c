...
#define LED_ID_CUSTOM_STATE		1	/* RGB LED - Green */
...
static bool custom_led_activity = false;
static bool uart_led_activity = false;
static bool dap_led_activity = false;
static led_state_t custom_led_state = LED_STATE_OFF;
...
void set_custom_led_state(led_state_t state)
{
    custom_led_activity = true;
...
static void led_state_timer_expiry_fn(struct k_timer *timer)
{
	if (custom_led_activity) {

		static uint8_t custom_led_level = 0;

		if (custom_led_state == LED_STATE_ON) {

			custom_led_level = LED_MAX_BRIGHTNESS;

		}else if ((custom_led_state == LED_STATE_BLINK) || (custom_led_state == LED_STATE_BLINK_PERMANENT)) {
			// Toggle LED level
			custom_led_level = custom_led_level > 0 ? 0 : LED_MAX_BRIGHTNESS;

			// If in blink mode stop after one cycle
			if ((custom_led_level == 0) && (custom_led_state == LED_STATE_BLINK)) {
				custom_led_activity = false;
				custom_led_state = LED_STATE_OFF;
			}
		} else if (custom_led_state == LED_STATE_BREATH) {

			static uint8_t custom_led_dir = 1U;

			if (custom_led_dir) {
				custom_led_level += LED_BREATH_STEP;
				if (custom_led_level > LED_MAX_BRIGHTNESS) {
					custom_led_level -= LED_BREATH_STEP;
					custom_led_dir = 0U;
				}
			} else {
				if (custom_led_level > LED_BREATH_STEP) {
					custom_led_level -= LED_BREATH_STEP;
				} else {
					custom_led_level = 0;
					custom_led_dir = 1U;
		...
		// update LED
		(void) led_set_brightness(led_pwm, LED_ID_CUSTOM_STATE, custom_led_level);
	}
