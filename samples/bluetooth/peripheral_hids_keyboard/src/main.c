...
/* Key used to accept passkey value */
...
static void pairing_process(struct k_work *work)
{
	...
	if (IS_ENABLED(CONFIG_SOC_SERIES_NRF54HX) || IS_ENABLED(CONFIG_SOC_SERIES_NRF54LX)) {
		printk("Press USR Button to confirm.\n");
	}
}
...
static void caps_lock_handler(const struct bt_hids_rep *rep)
{
	...
	printk("Caps Lock is %s\n", report_val? "pressed" : "released");
}
...
static void button_changed(uint32_t button_state, uint32_t has_changed)
{
	...
	if (pairing_button_pressed &&
	    (has_changed & KEY_PAIRING_ACCEPT)) {
...
int main(void)
{
	...
	for (;;) {
		if (is_adv) {
			dk_set_led(BT_STATUS_LED, (++blink_status) % 2);
		}
